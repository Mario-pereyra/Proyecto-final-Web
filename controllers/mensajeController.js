...
exports.getConversations = async (req, res) => {
    try {
        ...
        const conversations = await mensajeRepository.getConversationsByUserId(usuarioId);
...
exports.getMessages = async (req, res) => {
    try {
        ...
        if (!conversacionId || !usuarioId) {
            return res.status(400).json({ message: 'El ID de conversación y el ID de usuario son requeridos.' });
        }
        ...
        const messages = await mensajeRepository.getMessagesByConversationId(conversacionId);
...
exports.createMessage = async (req, res) => {
    try {
        ...
        if (!conversacionId || !emisorId || !contenido) {
            return res.status(400).json({ message: 'Faltan datos requeridos (conversacionId, emisorId, contenido).' });
        }

        const result = await mensajeRepository.createMessage(conversacionId, emisorId, contenido);
        ...
        res.status(201).json({ message: 'Mensaje creado con éxito.', mensajeId: result.insertId });
...
exports.startConversation = async (req, res) => {
    try {
        ...
        if (!anuncioId || !compradorId || !vendedorId) {
            return res.status(400).json({ message: 'Faltan datos requeridos (anuncioId, compradorId, vendedorId).' });
        }
        ...
        const conversacionId = await mensajeRepository.findOrCreateConversation(anuncioId, compradorId, vendedorId);
