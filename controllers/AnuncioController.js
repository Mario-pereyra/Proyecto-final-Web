...
exports.createAnuncio = async (req, rep) => {
  ...
  try {
    ...
    return rep.status(201).json({
      message: "Anuncio creado exitosamente", anuncioId: createAnuncioResultado.insertId,
    });
  } catch (error) {
    console.error(error);
...
exports.updateAnuncio = async (req, rep) => {
  const anuncioId = req.params.anuncioId;
  const { titulo, descripcion, precio, categoriaId, subcategoriaId, estado, estado_publicacion, departamentoId, ciudadId,
    zona, vistas, valoracion, } = req.body;

  try {
    const updateAnuncioResultado = await anuncioRepository.updateAnuncio(anuncioId, titulo, descripcion, precio,
      categoriaId, subcategoriaId, estado, estado_publicacion, departamentoId, ciudadId, zona, vistas, valoracion);
...
exports.createAnuncioConImagenes = async (req, res) => {
  try {
    ...
    const { usuarioId, titulo, descripcion, precio, categoriaId, subcategoriaId, estado, estado_publicacion,
      departamentoId, ciudadId, zona, vistas, valoracion, } = req.body;
    const files = req.files || [];

    const anuncioResult = await anuncioRepository.createAnuncio(usuarioId, titulo, descripcion, precio,
      categoriaId, subcategoriaId, estado, estado_publicacion, departamentoId, ciudadId, zona, vistas, valoracion);
...
exports.buscarAnuncios = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error interno del servidor al buscar anuncios." });
  }
};
...
exports.updateEstadoPublicacion = async (req, rep) => {
  ...
  if (!estadosValidos.includes(estado_publicacion)) {
    return rep.status(400).json({ message: "Estado de publicación no válido. Opciones: activo, inactivo, vendido.", });
  }
  try {
    const updateEstadoResultado = await anuncioRepository.updateEstadoPublicacion(anuncioId, estado_publicacion);
    if (!updateEstadoResultado || updateEstadoResultado.affectedRows === 0) {
      return rep.status(404).json({ message: `No se encontró el anuncio con el id ${anuncioId}` });
    }
    return rep.status(200).json({ message: `Estado de publicación actualizado a '${estado_publicacion}'` });
  } catch (error) {
    console.error(error);
    return rep.status(500).json({ message: "Error al actualizar el estado de publicación" });
  }
};
...
exports.updateAnuncioConImagenes = async (req, rep) => {
  ...
  if (!anuncioId || isNaN(anuncioId) || Number(anuncioId) <= 0) {
    return rep.status(400).json({ message: "El ID de anuncio es requerido y debe ser un número válido y positivo." });
  }
  ...
  if (!precio || isNaN(precio) || Number(precio) < 0) {
    return rep.status(400).json({ message: "El precio es obligatorio y debe ser un número válido." });
  }
  if (!categoriaId || isNaN(categoriaId)) {
    return rep.status(400).json({ message: "La categoría es obligatoria y debe ser un número." });
  }
  if (!subcategoriaId || isNaN(subcategoriaId)) {
    return rep.status(400).json({ message: "La subcategoría es obligatoria y debe ser un número." });
  }
  if (!estado || typeof estado !== 'string' || !estado.trim()) {
    return rep.status(400).json({ message: "El estado es obligatorio." });
  }
  if (!estado_publicacion || typeof estado_publicacion !== 'string' || !estado_publicacion.trim()) {
    return rep.status(400).json({ message: "El estado de publicación es obligatorio." });
  }
  if (!departamentoId || isNaN(departamentoId)) {
    return rep.status(400).json({ message: "El departamento es obligatorio y debe ser un número." });
  }
  if (!ciudadId || isNaN(ciudadId)) {
    return rep.status(400).json({ message: "La ciudad es obligatoria y debe ser un número." });
  }
  ...
  imagenesIdaEliminar = imagenesIdaEliminar.filter(id => !isNaN(id) && Number(id) > 0);
  ...
  try {
    ...
    return rep.status(200).json({ message: "Anuncio e imágenes actualizados correctamente" });
  } catch (error) {
    console.error(error);
    return rep.status(500).json({ message: "Error al actualizar el anuncio y sus imágenes" });
  }
};
